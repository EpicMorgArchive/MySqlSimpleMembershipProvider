...
namespace MySql.Web.Security {
    /// <summary>
    ///     Provides security and authentication features for ASP.NET Web Pages applications with MySql, including the ability
    ///     to create user accounts,
    ///     log users in and out, reset or change passwords, and perform related tasks.
    /// </summary>
    public static class MySqlWebSecurity {
        /// <summary>
        ///     Represents the key to the mySqlSecurityInheritedContextType value in the
        ///     <seealso cref="System.Configuration.ConfigurationManager.AppSettings" /> property.
        /// </summary>
        public const string MySqlSecurityInheritedContextType = "mySqlSecurityInheritedContextType";

        /// <summary>
        ///     Represents the key to the enableMySqlSimpleMembership value in the
        ///     <seealso cref="System.Configuration.ConfigurationManager.AppSettings" /> property.
        /// </summary>
        public const string EnableMySqlSimpleMembershipKey = "enableMySqlSimpleMembership";

        /// <summary>
        ///     Gets a value indicating whether the
        ///     <see>
        ///         <cref>M:InitializeDatabaseConnection</cref>
        ///     </see>
        ...
        ///     The <seealso cref="MySqlSimpleMembershipProvider" /> membership provider is not registered in the configuration of
        ///     your site.
        ...
        ///     username is empty.-or-username already has a
        ///     membership account.-or-password is empty.-or-password is too long.-or-The database operation failed.
        /// </exception>
        /// <exception cref="System.InvalidOperationException">
        ///     The <seealso cref="Initialized" /> method was not called.-or-
        ///     The <seealso cref="InitializeDatabaseConnection(string)" /> method was not called.-or-
        ...
        public static string CreateUserAndAccount(
            ...
            var v = propertyValues as IDictionary<string, object>;
            if ( v != null )
                values = new Dictionary<string, object>( v );
        ...
        /// <returns>
        ///     The date and time that the membership account was created, or <see cref="System.DateTime.MinValue" /> if the
        ...
        ///     <see cref="System.DateTime.MinValue" /> if an incorrect password has not been entered for this account.
        /// </returns>
        /// <exception cref="System.InvalidOperationException">
        ///     The <seealso cref="Initialized" /> method was not called.-or-
        ///     The <seealso cref="InitializeDatabaseConnection(string)" /> method was not called.-or-
        ...
        /// <returns>
        ///     The date and time when the password was most recently changed, or <see cref="System.DateTime.MinValue" /> if
        ///     the password has not been changed for this account.
        /// </returns>
        /// <exception cref="System.InvalidOperationException">
        ///     The <seealso cref="Initialized" /> method was not called.-or-
        ///     The <seealso cref="InitializeDatabaseConnection(string)" /> method was not called.-or-
        ...
        public static void InitializeDatabaseConnection( string connectionStringName ) {
            var connect = new DatabaseConnectionInfo {
        ...
        ///     The number of seconds to lock a user account after the number of password attempts
        ///     exceeds the value in the allowedPasswordAttempts parameter.
        /// </param>
        ...
        /// </param>
        /// <param name="interval">
        ///     The number of seconds to lock out a user account after the number of password attempts exceeds
        ///     the value in the allowedPasswordAttempts parameter.
        /// </param>
        /// <returns><c>true</c> if the membership account is locked; otherwise, <c>false</c>.</returns>
        /// <exception cref="System.InvalidOperationException">
        ///     The <seealso cref="Initialized" /> method was not called.-or-
        ...
        public static bool Login( string userName, string password, bool persistCookie = false ) {
            var provider = VerifyProvider();
            var success = provider.ValidateUser( userName, password );
            if ( success ) FormsAuthentication.SetAuthCookie( userName, persistCookie );
            return success;
        }
        ...
        public static void RequireRoles( params string[] roles ) {
            VerifyProvider();
            if ( roles.All( role => Roles.IsUserInRole( CurrentUserName, role ) ) ) return;
        ...
        internal static void InitializeMembershipProvider(
            MySqlSimpleMembershipProvider extendedMembership,
            DatabaseConnectionInfo connect ) {
            if ( extendedMembership.InitializeCalled ) throw new InvalidOperationException( Resources.Security_InitializeAlreadyCalled );
        ...
        internal static void InitializeRoleProvider( MySqlSimpleRoleProvider extendedRoles, DatabaseConnectionInfo connect ) {
            if ( extendedRoles.InitializeCalled ) throw new InvalidOperationException( Resources.Security_InitializeAlreadyCalled );
            extendedRoles.ConnectionInfo = connect;
            extendedRoles.InitializeCalled = true;
        }

        internal static bool IsAccountLockedOutInternal(
            MySqlSimpleMembershipProvider provider,
            string userName,
        ...
        internal static void PreAppStartInit() {
            ...
            if ( builtInMembership != null ) {
                var mySqlSimpleMembership = CreateDefaultExtendedMembershipProvider( BuiltInMembershipProviderName, builtInMembership );
            ...
            if ( builtInRoles == null ) return;
            var MySqlSimpleRoles = CreateDefaultExtendedRoleProvider( BuiltInRolesProviderName, builtInRoles );
            Roles.Providers.Remove( BuiltInRolesProviderName );
            Roles.Providers.Add( MySqlSimpleRoles );
        }

        private static MySqlSimpleMembershipProvider CreateDefaultExtendedMembershipProvider(
            string name,
        ...
        private static MySqlSimpleRoleProvider CreateDefaultExtendedRoleProvider( string name, RoleProvider currentDefault ) {
            var roleProvider = new MySqlSimpleRoleProvider( currentDefault );
            var config = new NameValueCollection();
        ...
        private static void InitializeProviders( DatabaseConnectionInfo connect ) {
            var extendedMembership = System.Web.Security.Membership.Provider as MySqlSimpleMembershipProvider;
            if ( extendedMembership != null ) InitializeMembershipProvider( extendedMembership, connect );

            var extendedRoles = Roles.Provider as MySqlSimpleRoleProvider;
            if ( extendedRoles != null ) InitializeRoleProvider( extendedRoles, connect );
        ...
        private static MySqlSimpleMembershipProvider VerifyProvider() {
            var provider = System.Web.Security.Membership.Provider as MySqlSimpleMembershipProvider;
            if ( provider == null ) throw new InvalidOperationException( Resources.Security_NoExtendedMembershipProvider );

            provider.VerifyInitialized();
