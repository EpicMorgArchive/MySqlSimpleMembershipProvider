...
namespace MySql.Web.Security {
    ...
    public class MySqlSimpleMembershipProvider : ExtendedMembershipProvider {
        ...
        /// </summary>
        public MySqlSimpleMembershipProvider() : this( null ) {}
        ...
        public MySqlSimpleMembershipProvider( MembershipProvider previousProvider ) {
            ...
            if ( this._previousProvider != null ) {
                this._previousProvider.ValidatingPassword += ( sender, args ) => {
                    if ( !this.InitializeCalled ) this.OnValidatingPassword( args );
                };
            }
        }
        ...
        public override bool EnablePasswordRetrieval {
            get {
                return !this.InitializeCalled && this.PreviousProvider.EnablePasswordRetrieval;
            }
        }
        ...
        public override bool EnablePasswordReset {
            get {
                return !this.InitializeCalled && this.PreviousProvider.EnablePasswordReset;
            }
        }
        ...
        public override bool RequiresQuestionAndAnswer {
            get {
                return !this.InitializeCalled && this.PreviousProvider.RequiresQuestionAndAnswer;
            }
        }
        ...
        public override bool RequiresUniqueEmail {
            get {
                return !this.InitializeCalled && this.PreviousProvider.RequiresUniqueEmail;
            }
        }
        ...
        /// <returns>
        ///     One of the <see cref="T:System.Web.Security.MembershipPasswordFormat" /> values indicating the format for
        ///     storing passwords in the data store.
        /// </returns>
        ...
        /// <returns>
        ///     The number of minutes in which a maximum number of invalid password or password-answer attempts are allowed
        ///     before the membership user is locked out.
        /// </returns>
        ...
        ///     Not initialized MySqlSimpleMembershipProvider.
        ///     This provider initialized call by
        ///     <seealso cref="MySql.Web.Security.MySqlWebSecurity.InitializeDatabaseConnection(string)" /> or
        ///     <seealso cref="MySql.Web.Security.MySqlWebSecurity.InitializeDatabaseConnection(string, string)" /> methods.
        /// </exception>
        ...
        public override bool ChangePassword( string username, string oldPassword, string newPassword ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.ChangePassword( username, oldPassword, newPassword );

            // REVIEW: are commas special in the password?
            if ( username.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "username" );
            if ( oldPassword.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "oldPassword" );
            if ( newPassword.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "newPassword" );
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                if ( !this.CheckPassword( db, userId, oldPassword ) ) return false;
        ...
        /// </summary>
        /// <returns>
        ///     True if the account could be successfully confirmed. False if the username was not found or the confirmation
        ///     token is invalid.
        /// </returns>
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                var membership =
                    db.Memberships.SingleOrDefault(
                        x => x.UserProfile.UserName == userName && x.ConfirmationToken == accountConfirmationToken );
        ...
        ///     There is a tiny possibility where this method fails to work correctly. Two or more users could be assigned the same
        ///     token but specified using different cases.
        ...
        public override bool ConfirmAccount( string accountConfirmationToken ) {
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                var membership =
                    db.Memberships.SingleOrDefault( x => x.ConfirmationToken.Equals( accountConfirmationToken, StringComparison.Ordinal ) );
        ...
        /// <summary>
        ///     When overridden in a derived class, creates a new user account using the specified user name and password,
        ///     optionally requiring that the new account must be confirmed before the account is available for use.
        /// </summary>
        /// <remarks>Inherited from ExtendedMembershipProvider ==> Simple Membership MUST be enabled to use this method</remarks>
        /// <param name="userName">The user name.</param>
        /// <param name="password">The password.</param>
        /// <param name="requireConfirmationToken">
        ///     (Optional) true to specify that the account must be confirmed; otherwise, false.
        ...
        ///     The user was not created. Check the <seealso cref="System.Web.Security.MembershipCreateUserException.StatusCode" />
        ///     property for a <seealso cref="System.Web.Security.MembershipCreateStatus" /> value.
        /// </exception>
        public override string CreateAccount( string userName, string password, bool requireConfirmationToken ) {
            this.VerifyInitialized();

            if ( password.IsEmpty() ) throw new MembershipCreateUserException( MembershipCreateStatus.InvalidPassword );
            ...
            if ( userName.IsEmpty() ) throw new MembershipCreateUserException( MembershipCreateStatus.InvalidUserName );

            using ( var db = this.NewMySqlSecurityDbContext ) {
                // Step 1: Check if the user exists in the Users table
                var uid = GetUserId( db, userName );
                if ( uid == -1 ) {
                    // User not found
                ...
                if ( result > 0 ) throw new MembershipCreateUserException( MembershipCreateStatus.DuplicateUserName );
                ...
                var insert = db.SaveChanges();

                if ( insert != 1 ) throw new MembershipCreateUserException( MembershipCreateStatus.ProviderError );
                return token;
            }
        }

        /// <summary>
        ///     When overridden in a derived class, creates a new OAuth membership account, or updates an existing OAuth Membership
        ///     account.
        /// </summary>
        /// <param name="provider">The OAuth or OpenID provider.</param>
        /// <param name="providerUserId">
        ///     The OAuth or OpenID provider user ID. This is not the user ID of the user account, but the
        ///     user ID on the OAuth or Open ID provider.
        /// </param>
        /// <param name="userName">The user name.</param>
        /// <exception cref="System.Web.Security.MembershipCreateUserException">
        ///     The user was not created. Check the <seealso cref="System.Web.Security.MembershipCreateUserException.StatusCode" />
        ...
        public override void CreateOrUpdateOAuthAccount( string provider, string providerUserId, string userName ) {
            ...
            if ( userName.IsEmpty() ) throw new MembershipCreateUserException( MembershipCreateStatus.ProviderError );

            var userId = this.GetUserId( userName );
            if ( userId == -1 ) throw new MembershipCreateUserException( MembershipCreateStatus.InvalidUserName );

            var oldUserId = this.GetUserIdFromOAuth( provider, providerUserId );
            using ( var db = this.NewMySqlSecurityDbContext ) {
                if ( oldUserId == -1 ) {
                    db.OAuthMemberships.Add(
                        new OAuthMembership {
                            Provider = provider,
                    ...
                    var oAuthMembership =
                        db.OAuthMemberships.SingleOrDefault(
                            x => x.Provider.ToUpper() == provider && x.ProviderUserId.ToUpper() == providerUserId );
        ...
        /// </param>
        /// <returns>
        ///     A <see cref="T:System.Web.Security.MembershipUser" /> object populated with the information for the newly
        ///     created user.
        /// </returns>
        /// <exception cref="System.NotSupportedException">
        ///     Not initialized MySqlSimpleMembershipProvider.
        ...
        ///     (Optional) true to specify that the user account must be confirmed; otherwise, false.
        ///     The default is false.
        /// </param>
        /// <param name="values">
        ///     (Optional) A dictionary that contains additional user attributes to store in the user profile. The
        ///     default is null.
        /// </param>
        ...
        public override string CreateUserAndAccount(
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                this.CreateUserRow( db, userName, values );
        ...
        /// <summary>
        ///     When overridden in a derived class, deletes the OAuth or OpenID account with the specified provider name and
        ///     provider user ID.
        /// </summary>
        ...
        public override void DeleteOAuthAccount( string provider, string providerUserId ) {
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                var oAuthMembership =
                    db.OAuthMemberships.SingleOrDefault(
                        x => x.Provider.ToUpper() == provider.ToUpper() && x.ProviderUserId.ToUpper() == providerUserId.ToUpper() );
        ...
        public override bool DeleteUser( string username, bool deleteAllRelatedData ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.DeleteUser( username, deleteAllRelatedData );
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                var returnValue = ( db.SaveChanges() == 1 );
        ...
        public override string GeneratePasswordResetToken( string userName, int tokenExpirationInMinutesFromNow ) {
            ...
            if ( userName.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "userName" );
            using ( var db = this.NewMySqlSecurityDbContext ) {
                var userId = this.VerifyUserNameHasConfirmedAccount( db, userName, true );
                var membership =
        ...
        public override ICollection<OAuthAccountData> GetAccountsForUser( string userName ) {
            ...
            var userId = this.GetUserId( userName );
            if ( userId == -1 ) return new OAuthAccountData[0];
            using ( var db = this.NewMySqlSecurityDbContext ) {
                var records =
                    db.OAuthMemberships.Where( x => x.UserId == userId )
                      .ToArray()
        ...
        public override MembershipUserCollection GetAllUsers( int pageIndex, int pageSize, out int totalRecords ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetAllUsers( pageIndex, pageSize, out totalRecords );
            throw new NotSupportedException();
        }
        ...
        /// <returns>
        ///     The date and time the account was created, or <see cref="F:System.DateTime.MinValue" /> if the account
        ...
        /// <summary>
        ///     When overridden in a derived class, returns the date and time when an incorrect password was most recently entered
        ...
        ///     <see cref="F:System.DateTime.MinValue" /> if an incorrect password has not been entered for this user account.
        /// </returns>
        /// <exception cref="System.InvalidOperationException">No user found.</exception>
        ...
        public override int GetNumberOfUsersOnline() {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetNumberOfUsersOnline();
            throw new NotSupportedException();
        }
        ...
        public override string GetPassword( string username, string answer ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetPassword( username, answer );
            throw new NotSupportedException();
        }

        /// <summary>
        ///     When overridden in a derived class, returns the date and time when the password was most recently changed for the
        ///     specified membership account.
        /// </summary>
        /// <remarks>Inherited from ExtendedMembershipProvider ==> Simple Membership MUST be enabled to use this method</remarks>
        /// <param name="userName">The user name of the account.</param>
        /// <returns>
        ///     The date and time when the password was more recently changed for membership account, or
        ///     <see cref="F:System.DateTime.MinValue" /> if the password has never been changed for this user account.
        /// </returns>
        /// <exception cref="System.InvalidOperationException">No user found.</exception>
        ...
        /// <summary>
        ///     When overridden in a derived class, returns the number of times that the password for the specified user account
        ///     was incorrectly entered since the most recent successful login or since the user account was created.
        /// </summary>
        /// <remarks>Inherited from ExtendedMembershipProvider ==> Simple Membership MUST be enabled to use this method</remarks>
        /// <param name="userName">The user name of the account.</param>
        ...
        /// </param>
        /// <returns>
        ///     A <see cref="T:System.Web.Security.MembershipUser" /> object populated with the specified user's information
        ...
        public override MembershipUser GetUser( object providerUserKey, bool userIsOnline ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetUser( providerUserKey, userIsOnline );
            throw new NotSupportedException();
        }
        ...
        public override MembershipUser GetUser( string username, bool userIsOnline ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetUser( username, userIsOnline );

            // Due to a bug in v1, GetUser allows passing null / empty values.
            using ( var db = this.NewMySqlSecurityDbContext ) {
                var userId = GetUserId( db, username );
                if ( userId == -1 ) return null; // User not found
        ...
        /// <summary>
        ///     When overridden in a derived class, returns the user ID for the specified OAuth or OpenID provider and provider
        ///     user ID.
        /// </summary>
        /// <param name="provider">The name of the OAuth or OpenID provider.</param>
        /// <param name="providerUserId">
        ...
        public override string GetUserNameByEmail( string email ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.GetUserNameByEmail( email );
            throw new NotSupportedException();
        }
        ...
        /// </summary>
        /// <remarks>
        ///     Inherited from ProviderBase - The "previous provider" we get has already been initialized by the Config
        ///     system, so we shouldn't forward this call
        /// </remarks>
        ...
        ///     An attempt is made to call Initialize on a provider after the
        ///     provider has already been initialized.
        /// </exception>
        public override void Initialize( string name, NameValueCollection config ) {
            if ( config == null ) throw new ArgumentNullException( "config" );
            if ( String.IsNullOrEmpty( name ) ) name = "MySqlSimpleMembershipProvider";
        ...
        /// <summary>
        ///     When overridden in a derived class, returns a value that indicates whether the user account has been confirmed by
        ///     the provider.
        /// </summary>
        /// <remarks>Inherited from ExtendedMembershipProvider ==> Simple Membership MUST be enabled to use this method</remarks>
        /// <param name="userName">The user name.</param>
        ...
        public override bool IsConfirmed( string userName ) {
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                var userId = this.VerifyUserNameHasConfirmedAccount( db, userName, false );
                return ( userId != -1 );
            }
        }
        ...
        public override string ResetPassword( string username, string answer ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.ResetPassword( username, answer );
            throw new NotSupportedException();
        }

        /// <summary>
        ///     When overridden in a derived class, resets a password after verifying that the specified password reset token is
        ///     valid.
        /// </summary>
        /// <remarks>Inherited from ExtendedMembershipProvider ==> Simple Membership MUST be enabled to use this method</remarks>
        ...
        public override void StoreOAuthRequestToken( string requestToken, string requestTokenSecret ) {
            ...
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                var insert = db.SaveChanges();
                if ( insert != 1 ) throw new ProviderException( Resources.SimpleMembership_FailToStoreOAuthToken );
            }
        }
        ...
        public override bool UnlockUser( string userName ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.UnlockUser( userName );
            throw new NotSupportedException();
        }
        ...
        public override void UpdateUser( MembershipUser user ) {
            if ( !this.InitializeCalled ) this.PreviousProvider.UpdateUser( user );
        ...
        public override bool ValidateUser( string username, string password ) {
            if ( !this.InitializeCalled ) return this.PreviousProvider.ValidateUser( username, password );
            if ( username.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "username" );
            if ( password.IsEmpty() ) throw new ArgumentException( CommonResources.Argument_Cannot_Be_Null_Or_Empty, "password" );
        ...
        public int GetUserId( string userName ) {
            ...
            return GetUserId( this.NewMySqlSecurityDbContext, userName );
        }
        ...
        internal bool DeleteUserAndAccountInternal( string userName ) {
            return ( this.DeleteAccount( userName ) && this.DeleteUser( userName, false ) );
        }
        ...
        internal void ValidateTable() {
            using ( var db = this.NewMySqlSecurityDbContext ) {
                ...
                try {
                    ...
                    throw new InvalidOperationException(
                        String.Format( CultureInfo.InvariantCulture, Resources.Security_FailedToFindUserTable, "UserProfile" ),
                        e );
                }
            }
        }
        ...
        private static string GenerateToken() {
            using ( var prng = new RNGCryptoServiceProvider() ) return GenerateToken( prng );
        }
        ...
        private static bool SetPassword( MySqlSecurityDbContext db, int userId, string newPassword ) {
            ...
            if ( hashedPassword.Length > 128 ) throw new ArgumentException( Resources.SimpleMembership_PasswordTooLong );
        ...
        private bool CheckPassword( MySqlSecurityDbContext db, int userId, string password ) {
            var hashedPassword = this.GetHashedPassword( db, userId );
            ...
            if ( verificationSucceeded ) {
                ...
                var failures = GetPasswordFailuresSinceLastSuccess( db, userId );
                if ( failures != -1 ) {
                    if ( membership != null ) {
        ...
        private void CreateUserRow( MySqlSecurityDbContext db, string userName, IDictionary<string, object> values ) {
            ...
            if ( values != null && values.Count > 0 ) {
                ...
                var userProfileExType = contextAssembly.GetTypes().FirstOrDefault( x => x.BaseType == typeof( UserProfile ) );

                if ( userProfileExType != null ) {
                    var userProfileEx = Activator.CreateInstance( userProfileExType );
                    ...
                    addMethod.Invoke(
                        ...
                        new[] {
                            userProfileEx
                        } ); // invoke add UserProfile inherited class object
                }
            }
            else
            ...
            if ( rows != 1 ) throw new MembershipCreateUserException( MembershipCreateStatus.ProviderError );
        }
        ...
        private void CreateUserRowByDatabase( MySqlSecurityDbContext db, string userName, IDictionary<string, object> values ) {
            ...
            using ( var scope = new TransactionScope() ) {
                ...
                if ( values != null && values.Count > 0 ) {
                    var user = db.UserProfiles.OrderByDescending( x => x.UserName ).First( x => x.UserName == userName );
                    var sql = new StringBuilder( "UPDATE UserProfile SET " );
                    ...
                    var sqlCommand = string.Format( "{0} WHERE UserId = {1}", sql.ToString( 0, sql.Length - 1 ), user.UserId );
            ...
            if ( rows == 0 ) throw new MembershipCreateUserException( MembershipCreateStatus.ProviderError );
        }

        private object EntryBy( DbContext dbContext, string typeFullName ) {
            var contextType = dbContext.GetType();
            var assembly = Assembly.GetAssembly( contextType );
            var dacType = assembly.GetType( contextType.FullName );
